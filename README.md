# Motor Parts Processing System

A system for processing motor part numbers, cleaning them to match database formats, and finding parts information based on various classification criteria.

## Architecture

This implementation uses a pure Python procedural3. Look for embedded part numbers within the input
...
## Part Description Matchingach, replacing the previous LangGraph supervisor agent architecture for improved simplicity, performance, and cost-efficiency.

### Key Components

1. **PartProcessor** (`src/processor.py`): 
   - Main processing class that handles the entire workflow
   - Uses direct procedural code for better performance
   - Maintains all functionality with simplified implementation

2. **Pattern Optimizer** (`tools/pattern_optimizer.py`):
